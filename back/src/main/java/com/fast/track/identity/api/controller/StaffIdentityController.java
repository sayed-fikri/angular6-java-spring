 ...
 */
@RestController
@RequestMapping("/api/staff/identity")
public class StaffIdentityController {

    private static final Logger LOG = LoggerFactory.getLogger(StaffIdentityController.class);
    ...
    @Autowired
    public StaffIdentityController(IdentityService identityService, SecurityService securityService,
                                   IdentityTransformer identityTransformer, CommonService commonService,
                                   AuthenticationManager authenticationManager, EnrollmentService enrollmentService) {
    ...
    public ResponseEntity<Staff> findStaff() {
        return new ResponseEntity<Staff>(identityTransformer.toStaffVo(
                getCurrentStaff()), HttpStatus.OK);
    }
    ...
    @GetMapping(value = "/users-activate", params = {"email"})
    public ResponseEntity<ApplicationSuccess> activateUser(@RequestParam String email) {
        DexUser user = identityService.findUserByEmail(email);
    ...
    public ResponseEntity<Therapist> findStaffTherapistByCode(@PathVariable String code) {
    ...
    public ResponseEntity<ApplicationSuccess> removeTherapist(@PathVariable String code) {
        DexTherapist therapist = identityService.findTherapistByCode(code);
        List<DexConsultation> consultations = enrollmentService.findConsultationByTherapist(therapist);

        for(DexConsultation consultation : consultations){
            enrollmentService.removeConsultation(consultation);
            List<DexComment> comments = consultation.getComments();
            for(DexComment comment : comments){
    ...
    private DexStaff getCurrentStaff () {
        DexUser user = securityService.getCurrentUser();
        return (DexStaff) user.getActor();
    }
